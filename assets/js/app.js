...
const showSurah = (surahNumber) => {
  sidebar.classList.remove('active')
  showLoader()
    fetch(`/assets/quran/json/${surahNumber}`)
    .then(res => res.json())
    .then(res => {
      getSurah(res.data)
      hideLoader()
...
/*const showSurah = (surahNumber) => {
    ...
    $.ajax({
        ...
        type: 'POST',
        data: data,
        success: function (result, status, xhr) { /*hooray!*/ //},
  //      error: function (xhr, status, error) { /*houston, we have a problem!*/ }
//    });
//}*/
...
const getSurah = (surah) => {
  ...
  for (i; i < surah.verses.length; i++) {
    versesContainer.innerHTML += `<ul class="verse">
         <li class="arab">${a.verses[b].text.arab}  . ${formatter.format(a.verses[b].number.inQuran)}</li>
         <li class="arab">${a.verses[b].number.Jommal_Moderen} </li>
         <li class="arab">${a.verses[b].number.Jommal_classic} </li>
